Function Deploy-Service {
    ...
    if (!$ServiceOnly) {
        ...
        if (Test-Path "$ServiceSrcPath/postgres.config.json") {
            $postgresConfig = Get-Content -Path "$ServiceSrcPath/postgres.config.json" | ConvertFrom-Json -AsHashtable
            Deploy-PostgresConfig -CdfConfig $SvcCdfConfig -Scope 'Service' -SchemaName $postgresConfig.schemaName
        }
