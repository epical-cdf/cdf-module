Function Remove-RecoveryServicesVault {
    <#
        ...
        .DESCRIPTION
        It is common for enterprises to have policies that creates recovery services vaults for backup of storage accounts and VM disks etc.
        This command tries to remove all configurationas and recovery items and have the vault completely removed.
        ...
        $config | Remove-RecoveryServicesVault `
            -CdfConfig $config `
            -VaultName test `
    ...
    if (!$VaultToDelete) {
        Write-Warning "No Recovery Services Vault found [$VaultName]."
        return
    }
    Set-AzRecoveryServicesAsrVaultContext -Vault $VaultToDelete

    Set-AzRecoveryServicesVaultProperty -Vault $VaultToDelete.ID -SoftDeleteFeatureState Disable | Out-Null #disable soft delete
    Write-Verbose "Soft delete disabled for the vault" $VaultName
    ...
    foreach ($softitem in $containerSoftDelete) {
        Undo-AzRecoveryServicesBackupItemDeletion -Item $softitem -VaultId $VaultToDelete.ID -Force | Out-Null #undelete items in soft delete state
    }

    #Invoking API to disable Security features (Enhanced Security) to remove MARS/MAB/DPM servers.
    Set-AzRecoveryServicesVaultProperty -VaultId $VaultToDelete.ID -DisableHybridBackupSecurityFeature $true -ErrorAction:SilentlyContinue | Out-Null
    Write-Verbose "Disabled Security features for the vault"
        ...
        foreach ($fabricObject in $fabricObjects) {
            ...
            foreach ($containerObject in $containerObjects) {
                ...
                foreach ($protectedItem in $protectedItems) {
                    Write-Verbose "Triggering DisableDR(Purge) for item:" $protectedItem.Name
                    Remove-AzRecoveryServicesAsrReplicationProtectedItem -InputObject $protectedItem -Force
                    Write-Verbose "DisableDR(Purge) completed"
                }
                ...
                foreach ($containerMapping in $containerMappings) {
                    Write-Verbose "Triggering Remove Container Mapping: " $containerMapping.Name
                    Remove-AzRecoveryServicesAsrProtectionContainerMapping -ProtectionContainerMapping $containerMapping -Force
                    Write-Verbose "Removed Container Mapping."
                }
            }
            ...
            # Remove Fabric
            Write-Verbose "Triggering Remove Fabric:" $fabricObject.FriendlyName
            Remove-AzRecoveryServicesAsrFabric -InputObject $fabricObject -Force
            Write-Verbose "Removed Fabric."
        }
    }
    ...
    Write-Verbose "Removed Private Endpoints"
    ...
    $restUri = "https://management.azure.com/subscriptions/" + $CdfConfig.Platform.Env.subscriptionId + '/resourcegroups/' + $ResourceGroup + '/providers/Microsoft.RecoveryServices/vaults/' + $VaultName + '?api-version=2021-06-01&operation=DeleteVaultUsingPS'
