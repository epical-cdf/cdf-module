Function Update-ServiceLogicAppStd {
    <#
      .SYNOPSIS
    ...
    #############################################################
    # Update the service CDF Config file
    #############################################################

    $serviceConfig = Get-Content -Path "$ServicePath/cdf-config.json" | ConvertFrom-Json -AsHashtable
    ...
    $appSettings.Values.WORKFLOWS_LOCATION_NAME = $CdfConfig.Platform.Env.region
    $appSettings.Values.WORKFLOWS_RESOURCE_GROUP_NAME = $CdfConfig.Platform.ResourceNames.apiConnResourceGroupName

    $appSettings | ConvertTo-Json -Depth 5 | Set-Content -Force -Path "local.settings.json"
